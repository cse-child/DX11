...
void Transform::RenderUI()
{
	...
	temp = tag + "_Rot";
	Float3 rot;
	rot.x = XMConvertToRadians(localRotation.x);
	rot.y = XMConvertToRadians(localRotation.y);
	rot.z = XMConvertToRadians(localRotation.z);

	ImGui::DragFloat3(temp.c_str(), (float*)&localRotation, 1.0f, -180, 180);
