...
void InitDevice()
{
    ...
    for (UINT angle = 0; angle < polygonNum; angle++)
    {
        vertices.emplace_back(0, 0);

        x = cosf((angle + 1) * XM_2PI / polygonNum) * radius;
        y = sinf((angle + 1) * XM_2PI / polygonNum) * radius;
        vertices.emplace_back(x, y);
        
        x = cosf(angle * XM_2PI / polygonNum) * radius;
        y = sinf(angle * XM_2PI / polygonNum) * radius;
        vertices.emplace_back(x, y);
    }
